let reptQuery = {
  ...
  person_location_area: {
    ...
    sign: 1,
    needDisplay: 1,
    sqlTmpl: 'select {resultFields} from(select hlas.area_id, count(distinct hlas.obj_id) as per_nums from his_location_area_staff hlas left join dat_staff_extend dse ON dse.staff_id = hlas.obj_id where hlas.area_id > 0 {exprString} group by hlas.area_id) aa',
    fields: {
  ...
  person_reader: {
    ...
    fields: {
      ...
      types: ['NUMBER', 'NUMBER', 'STRING', 'SELECT', 'SELECT', 'STRING', 'SELECT', 'DATETIME', 'DATETIME', 'STRING'],
  ...
  person_well_overtime: {
    ...
    fields: {
      names: ['s.card_id', 'ds.name', 'ifnull(d.name, " ")', 's.occupation_id', 'ifnull(date_format(ras.start_time, "%Y-%m-%d %H:%i:%s"), " ") as start_time', 'hed.cur_time', 'ifnull(date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s"), " ") as end_time', 'hed.limit_value', 'round(hed.cur_value,2)', 'case when aa.card_id is null then "否" else "是" end as isbat'],
      types: ['NUMBER', 'STRING', 'STRING', 'SELECT', 'DATETIME', 'DATETIME', 'DATETIME', 'STRING', 'STRING', 'STRING'],
      labels: ['卡号', '姓名', '所属部门', '职务', '入井时间', '开始告警时间', '结束告警时间', '规定时长(分)', '实际时长(分)', '充放电']
    },
    exprFields: [{
      name: 'ds.staff_id',
      label: '员工编号',
  ...
  person_area_overtime: {
    ...
    fields: {
      names: ['hed.obj_id', 's.staff_id', 's.dept_id', 's.occupation_id', 'hed.area_id', 'hlas.enter_time', 'hed.cur_time', 'hed1.cur_time as end_time', 'round(hed.limit_value/60, 2) as limit_value', 'round(hed.cur_value/60,2) as cur_value'],
  ...
  person_area_overcount: {
    ...
    fields: {
      names: ['et.name', 'a.name as an', 'hed.limit_value', 'hed.cur_value', 'hed.cur_time', 'ifnull(date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s"), "") as end_time'],
  ...
  person_call_help: {
    ...
    fields: {
      names: ['s.card_id', 'ds.name', 's.dept_id', 's.occupation_id', 'date_format(ras.start_time, "%Y-%m-%d %H:%i:%s") as start_time', 'date_format(hed.cur_time, "%Y-%m-%d %H:%i:%s") as cur_time', 'date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s") as end_time'],
      types: ['STRING', 'STRING', 'SELECT', 'SELECT', 'DATETIME', 'DATETIME', 'DATETIME'],
  ...
  person_card_battery_alarm: {
    ...
    fields: {
      names: ['hed.obj_id as card_id', 'ds.staff_id', 'dd.name', 'ds.dept_id', 'et.name as et_name', 'hed.cur_time', 'hed1.cur_time as hedc', 'case when aa.card_id is null then "否" else "是" end as isbat'],
      types: ['NUMBER', 'NUMBER', 'STRING', 'SELECT', 'SELECT', 'DATETIME', 'DATETIME', 'STRING'],
      labels: ['卡号', '员工编号', '姓名', '所属部门', '告警类型', '开始告警时间', '结束告警时间', '充放电']
    },
    exprFields: [{
      name: 'ds.staff_id',
      label: '员工编号',
  ...
  v_area_limited: {
    ...
    fields: {
      names: ['hed.obj_id', 'ds.vehicle_id', 'dda.name', 'ds.vehicle_type_id', 's.dept_id', 'hed.area_id', 'da.area_type_id', 'hed.cur_time', 'ifnull(date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s"),"") as end_time', 'timestampdiff(minute, hed.cur_time, hed1.cur_time) as retime'],
  ...
  alarm_reader: {
    ...
    fields: {
      names: ['CAST(hed.obj_id AS signed) as obj_id', 'r.name', 'et.event_type_id', 'ifnull(date_format(hed.cur_time, "%Y-%m-%d %H:%i:%s"), "") as start_time', 'ifnull(date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s"), "") as end_time'],
  ...
  his_staff_change: {
    ...
    sign: 1,
    needDisplay: 1,
    sqlTmpl: `SELECT {resultFields} FROM his_staff_op_log hso LEFT JOIN dat_staff_extend dse ON dse.staff_id = hso.staff_id WHERE 1=1 AND (detail IS NOT NULL && detail != '') {exprString} order by dse.staff_id, last_update desc;`,
    fields: {
      names: ['hso.staff_id', 'CASE WHEN op_log = "INSERT" THEN "新增" WHEN op_log = "UPDATE" THEN "更新" WHEN op_log = "DELETE" THEN "删除" END as op_log', 'hso.detail', 'hso.user_id', 'hso.last_update'],
  ...
  his_alarm: {
    ...
    sqlTmpl: null,
    fields: {
      names: ['event_type_id', 'obj', 'objName', 'numberID', 'cardID', 'deptName', 'date_format(hed.cur_time, "%Y-%m-%d %H:%i:%s") as cur_time', 'date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s") as end_time'],
      types: ['SELECT', 'STRING', 'STRING', 'NUMBER', 'STRING', 'STRING', 'DATETIME', 'DATETIME'],
      labels: ['告警类型', '告警对象', '名称', '编号', '卡号', '部门', '开始告警时间', '结束告警时间']
    },
    exprFields: [
      {
        name: 'event_type_id',
