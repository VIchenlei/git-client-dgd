...
const queryTrackDef = {
  ...
  'notattendance': {
    'staff': {
      sql: `select bt.staff_id, bt.card_id, begin_time, ifnull(last_time, current_timestamp()) last_time, speed, begin_pt, hl.area_id, direction, location_flag from his_location_staff_ hl,dat_staff_extend bt where hl.obj_id=bt.staff_id and bt.staff_id IN ({objID}){timeExpr} ORDER BY begin_time;`,
      timeExpr: ` and begin_time >= "{startTime}" and begin_time <= "{endTime}"`
    },
    'vehicle': {
      sql: `select dve.vehicle_id, dve.card_id, begin_time, ifnull(last_time, current_timestamp()) last_time, speed, begin_pt, area_id,direction, location_flag from his_location_vehicle_ hlv, dat_vehicle_extend dve where hlv.obj_id = dve.vehicle_id and dve.vehicle_id = {objID}{timeExpr} order by begin_time;`,
      timeExpr: ` and begin_time >= "{startTime}" and begin_time < "{endTime}"`
    }
  }
}
