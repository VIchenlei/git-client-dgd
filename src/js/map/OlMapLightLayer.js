...
export default class OlMapLightLayer {
  ...
  initLayer () {
    ...
    this.registerEventHandler(this.map, this.lightLayer)
  }
  ...
  initDrawTrafficLisght(){
    let data =  xdata.metaStore.data.light && Array.from(xdata.metaStore.data.light.values()).map(item =>[item.lights_group_id, item.light_id, 10, ''])
  ...
  drawLightsAndChangeState(data){
    ...
    for (let i=0, len = data.length; i<len; i++) {
      ...
      let light = xdata.metaStore.data.light && xdata.metaStore.data.light.get(row[1])
      let physicsLightId = light && light.physics_light_id
      let isExitAlarm = alarms && alarms.find(item => Number(item.type_id) === 8 && Number(item.obj_id) === physicsLightId)
      if (isExitAlarm) continue
