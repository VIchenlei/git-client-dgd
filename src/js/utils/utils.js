...
function toTextBySelect (data, name, topicName) {
...
function changeToTextByType (type, data, name, topicName) {
  ...
  if (type === 'SELECT') return toTextBySelect(data, name, topicName)
...
function metaUpdateRes (res, topicName, cmd, nameList) {
  ...
  let dlgTips = null
  let updateRes = null
  if (res.data.name === topicName || nameList.includes(res.data.name)) {
    if (res.code === 0) {
      let resultText = null
      switch (cmd) {
        case 'INSERT':
...
function addBusinesslogic (value, self, evt) {
  ...
  let info = self.info
  const needAddlogic = ['dat_rules', 'his_maintenance', 'dat_map_gis', 'dat_area', 'dat_ip_address', 'dat_workface_scheduling', 'his_regular_cycle_detail', 'his_startup_detail', 'dat_sensor', 'dat_lights_group', 'dat_month', 'dat_reader', 'dat_gis_layer', 'tt_inspection_route_planning', 'dat_staff', 'dat_card_type', 'dat_worktype', 'dat_occupation']
  if (needAddlogic.includes(tableName)) {
    const checkLogic = {
      ...
      dat_map_gis: checkMapGis(name, value),
...
function filterItem (fieldValue, items, tableName, fieldName) {
  if (tableName === 'dat_reader' && fieldName === 'device_type_id' && fieldValue !== '') {
...
// 选择区域修改对应的区域列表
function changeCheckBox (currenTag) {
  let { fieldName } = currenTag
  let names = ['area_type_id', 'role_rank_id', 'card_type_id']
  if (!names.includes(fieldName)) return
  let input = currenTag.refs[fieldName]
  let selectedValue = input && input.getAttribute('name')
  selectedValue = Number(selectedValue) || currenTag.index
  let parenTag = currenTag.parent
  ...
  if (parenTag.tableName === 'dat_card' && fieldName === 'card_type_id') {
    parenTag.refs['card_id'].refs['card_id'].value = `00${selectedValue}`
  }
}
...
function filterTeam (rows, name, type) {
  rows = rows && rows.filter(dept => {
    let deptID = dept.dept_id || xdata.metaStore[name].get(dept[type]).dept_id
    if (!xdata.depts || xdata.depts.includes(deptID)) return dept
  })
  return rows
}
...
function getSerachData (fieldName, datas, self) {
  ...
  let name = fieldName
  const names = ['staffs', 'drivers', 'staff_id', 'vehicles']
  ...
  rows = names.includes(fieldName) ? Array.from(xdata.metaStore[name].values()) : xdata.metaStore.data[name] && Array.from(xdata.metaStore.data[name].values())
...
function getDisPlayList (rows, key, tableName) {
  let list = []
  if (tableName === 'his_leader_arrange') return rows
