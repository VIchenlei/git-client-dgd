...
const updateDef = {
  'power': {
    ...
    name: 'power',
    fields: {
      labels: ['设备地址', '设备类型', '强制放电', '交流供电状态', '交流供电电压（v）', '电源箱内温度（℃）', '电池当前电压（v）', '电池当前电流（mA）', '电池当前电量', '电池当前温度（℃）', '电池累计充放电次数', '电池累计充放电时间（h）', '功率器件当前温度（℃）', '直流输出电压（v）'],
      types: ['NUMBER', 'STRING', 'NUMBER', 'COLOR', 'NUMBER', 'STRING', 'NUMBER', 'NUMBER', 'COLOR', 'STRING', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER'],
      names: ['deviceAddress', 'deviceType', 'power_rode', 'excharge_state', 'supply_voltage', 'temperature', 'voltage', 'electricity', 'power', 'temperature', 'charge_discharge_num', 'charge_discharge_time', 'power_temperature', 'dc_voltage'],
      enableNull: [true, true, true, true, true, true, true, true, true, true, true, true, true, true]
    }
  }
}
